...
# Set the variables for the route: Delhi to Bangalore
source_airport = 'DEL'  # IATA code for Delhi
destination_airport = 'BLR'  # IATA code for Bangalore
...
airport_coordinates = {
    'DEL': (28.5562, 77.1000),  # Delhi coordinates
...
plt.plot([airport_coordinates['DEL'][1], airport_coordinates['BLR'][1]],
         [airport_coordinates['DEL'][0], airport_coordinates['BLR'][0]], color='red', linewidth=2)

# Annotate the airports
plt.annotate('Delhi (DEL)', xy=airport_coordinates['DEL'], xytext=(airport_coordinates['DEL'][1] + 0.5, airport_coordinates['DEL'][0] + 0.5),
...
plt.xlim([76.5, 78])  # Adjusting x-axis limits for better view
plt.ylim([12, 29])    # Adjusting y-axis limits for better view
plt.grid()
plt.show()
