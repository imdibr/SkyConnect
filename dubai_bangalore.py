...
# Set the variables for the route: Bangalore to Dubai
source_airport = 'BLR'  # IATA code for Bangalore
destination_airport = 'DXB'  # IATA code for Dubai
...
airport_coordinates = {
    ...
    'DXB': (25.276987, 55.396688)  # Dubai coordinates
}
...
plt.plot([airport_coordinates['BLR'][1], airport_coordinates['DXB'][1]],
         [airport_coordinates['BLR'][0], airport_coordinates['DXB'][0]], color='red', linewidth=2)
...
plt.annotate('Dubai (DXB)', xy=airport_coordinates['DXB'], xytext=(airport_coordinates['DXB'][1] + 0.5, airport_coordinates['DXB'][0] + 0.5),
             arrowprops=dict(facecolor='black', shrink=0.05))

plt.xlim([55, 56])  # Adjusting x-axis limits for better view
plt.ylim([25, 26])   # Adjusting y-axis limits for better view
plt.grid()
plt.show()
